<template>
 <div class="column-4 w-col w-col-8">
                <div class="div-block-425">
                  <div class="div-block-432">
                    <div class="columns w-row">
                      <div class="column w-col w-col-8">
                        <h3>
                          <a
                            href="https://angel.co/re/story/39313"
                            target="_blank"
                            class="link-15"
                            >This fingertip for robots uses magnets to 'feel'
                            things</a
                          >
                        </h3>
                        <div class="div-block-426">
                          <div class="div-block-431">
                            <a href="#" class="link-17"
                              >AI &amp; MACHINE LEARNING</a
                            >
                          </div>
                          <div class="div-block-427">
                            <div class="text-block-6">•</div>
                            <div>MATT SIMON</div>
                          </div>
                        </div>
                      </div>
                      <div class="column-2 w-col w-col-4">
                        <div class="div-block-443">
                          <img
                            src="https://uploads-ssl.webflow.com/5fc7827fd57bce892e4c852f/5fc8cdda354d643356c56b92_Virtual%20office%20boy.svg"
                            loading="lazy"
                            height=""
                            alt=""
                            class="image-11"
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div>
                    <p>
                      In 2015, venture capitalist Chris Sacca was one of Silicon
                      Valley’s most influential investors, No. 3 on the Midas
                      List with more than one million Twitter followers well
                      before VC memes were the norm, and a regular appearance on
                      ‘Shark Tank’. The Lowercase Capital partner’s early stakes
                      in companies including Instagram, Twitter and Uber minted
                      him a billionaire, putting him on the cover of Forbes.<a
                        href="#"
                      ></a>
                    </p>
                  </div>
                  <div class="div-block-428">
                    <div class="div-block-454">
                      <div class="html-embed-10 w-embed">
                        <img
                          src="data:image/svg+xml;base64,"
                        />
                      </div>
                      <div class="text-block-12">123</div>
                    </div>
                    <div class="div-block-429">
                      <div class="html-embed-8 w-embed">
                        <img
                          src="data:image/svg+xml;base64,"
                        />
                      </div>
                      <div class="html-embed-8 w-embed">
                        <img
                          src="data:image/svg+xml;base64,"
                        />
                      </div>
                    </div>
                    <div class="div-block-430">
                      <a href="#" class="link-16">Hace 3 dias</a>
                    </div>
                  </div>
                </div>
                <div class="div-block-425">
                  <div class="div-block-432">
                    <div class="columns w-row">
                      <div class="column w-col w-col-8">
                        <h3>
                          <a
                            href="https://angel.co/re/story/39313"
                            target="_blank"
                            class="link-15"
                            >This fingertip for robots uses magnets to 'feel'
                            things</a
                          >
                        </h3>
                        <div class="div-block-426">
                          <div class="div-block-431">
                            <a href="#" class="link-17"
                              >AI &amp; MACHINE LEARNING</a
                            >
                          </div>
                          <div class="div-block-427">
                            <div class="text-block-6">•</div>
                            <div>MATT SIMON</div>
                          </div>
                        </div>
                      </div>
                      <div class="column-2 w-col w-col-4">
                        <div class="div-block-443">
                          <img
                            src="https://uploads-ssl.webflow.com/5fc7827fd57bce892e4c852f/5fc7cedb80fe1013512ee3a5_Podcast-pana.svg"
                            loading="lazy"
                            height=""
                            alt=""
                            class="image-11"
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div>
                    <p>
                      In 2015, venture capitalist Chris Sacca was one of Silicon
                      Valley’s most influential investors, No. 3 on the Midas
                      List with more than one million Twitter followers well
                      before VC memes were the norm, and a regular appearance on
                      ‘Shark Tank’. The Lowercase Capital partner’s early stakes
                      in companies...<a href="#"></a>
                    </p>
                  </div>
                  <div class="div-block-428">
                    <div class="div-block-454">
                      <div class="html-embed-10 w-embed">
                        <img
                          src="data:image/svg+xml;base64,"
                        />
                      </div>
                      <div class="text-block-12">123</div>
                    </div>
                    <div class="div-block-429">
                      <div class="html-embed-8 w-embed">
                        <img
                          src="data:image/svg+xml;base64,"
                        />
                      </div>
                      <div class="html-embed-8 w-embed">
                        <img
                          src="data:image/svg+xml;base64,"
                        />
                      </div>
                    </div>
                    <div class="div-block-430">
                      <a href="#" class="link-16">Hace 3 dias</a>
                    </div>
                  </div>
                </div>
              </div>
</template>


<script>

export default {
    name: "historiasDestacadas", 
    data() {
        return {

        }
    },
}
</script>
